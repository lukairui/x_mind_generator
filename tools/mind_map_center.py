...
class MindMapCenterTool(Tool):
    ...
    def _draw_text_with_pil(self, img, draw, x, y, text, depth_level, color, font_file):
        ...
        try:
            ...
            # 智能文本换行，确保显示所有重要内容
            # 根据节点深度调整每行字符数 - 增加字符限制确保演员名称完整显示
            if depth_level == 1:
                max_chars_per_line = 18  # 根节点可以更长
                max_lines = 2
            elif depth_level == 2:
                max_chars_per_line = 16  # 二级节点（电影名称）
                max_lines = 3
            else:
                max_chars_per_line = 20  # 三级节点（演员信息）- 增加到20字符确保完整显示
                max_lines = 4  # 增加到4行确保多个演员都能显示
            
            if len(safe_text) > max_chars_per_line:
                # 优先在逗号处分割，保持演员名称完整
                lines = []
                
                # 首先尝试在逗号处分割
                if ',' in safe_text:
                    comma_parts = safe_text.split(',')
                    current_line = ""
                    ...
                    while current_pos < len(safe_text):
                        end_pos = min(current_pos + max_chars_per_line, len(safe_text))
                        
                        if end_pos == len(safe_text):
                            lines.append(safe_text[current_pos:])
                            break
                        ...
                        for i in range(end_pos - 1, max(current_pos, end_pos - 6), -1):
                            if safe_text[i] in break_chars:
                                break_pos = i + 1
                                break
                        
                        if break_pos > current_pos:
                            lines.append(safe_text[current_pos:break_pos].strip())
                            current_pos = break_pos
                        else:
                            # 强制断行
                            lines.append(safe_text[current_pos:end_pos])
                            current_pos = end_pos
    ...
    def _generate_png_mindmap(self, tree_data: dict, output_file: str, temp_dir: str) -> bool:
        ...
        try:
            ...
            def layout_dynamic_center_mindmap(node, center_x=0, center_y=0, depth_level=1, parent_angle=0, angle_range=2*math.pi, inherited_color='#333333'):
                ...
                # Calculate radius - 增大半径避免重叠
                base_radius = 4.5  # 增大基础半径
                depth_factor = 0.8  # 增大层级因子
                child_factor = 0.15  # 增大子节点因子
                radius = base_radius + (depth_level * depth_factor) + (child_count * child_factor)
                radius = min(radius, axis_limit * 0.4)  # 增大最大半径限制
                
                # Calculate angles - 优化角度分配避免重叠
                if child_count == 1:
                    ...
                    if depth_level == 1:
                        ...
                        # 非根节点：在分配的角度范围内分布，确保足够间距
                        if child_count > 1:
                            # 为避免重叠，确保最小角度间距
                            min_angle_spacing = math.pi / 8  # 最小22.5度间距
                            required_range = (child_count - 1) * min_angle_spacing
                            actual_range = min(angle_range, required_range * 1.2)  # 留20%余量
                            
                            start_angle = parent_angle - actual_range / 2
                            angle_step = actual_range / max(child_count - 1, 1)
                        else:
